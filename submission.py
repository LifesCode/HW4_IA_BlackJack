    ...
    def succAndProbReward(self, state: Tuple, action: str) -> List[Tuple]:
        ...
        if action == 'Take':
            ...
            if nextCard is None:
                for i, card in enumerate(deckCard):
                    if card:
                        successors += [((totalValue, i, deckCard), float(card) / totalCard, -self.peekCost)]
            return successors
        return [((totalValue, None, None), 1, totalValue)]  # action taken was to Quit
...
def blackjackFeatureExtractor(state: Tuple, action: str) -> List[tuple]:
    ...
    if counts is not None:
        featureKey = [1 if x else 0 for x in counts]  # list(map(lambda x: 1 if x != 0 else 0, counts))
        feature += [(tuple(['cardPresence'] + featureKey + [action]), 1)]
        for i in range(len(counts)):
            feature += [(('cardAndAction', i, counts[i], action), 1)]
    return feature
    # END_YOUR_CODE
